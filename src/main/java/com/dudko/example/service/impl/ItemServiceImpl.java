...
public class ItemServiceImpl implements ItemService {
    ...
    public Item getById(@NonNull String id) {
        log.debug("Fetching Data from Elasticsearch...");
    ...
    @Override
    public List<Item> getByContent(String term) {
        return repository.findAllByContent(term).stream()
                .map(mapper::mapToModel)
                .toList();
    }
    ...
    public List<Item> getAll(int pageNumber, int pageSize) {
        log.debug("Fetching Data from Elasticsearch...");
        return repository.findAll(PageRequest.of(pageNumber, pageSize)).getContent().stream()
                .map(mapper::mapToModel)
                .toList();
    }
